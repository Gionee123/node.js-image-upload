...
exports.create = async (request, response) => {
    ...
    // ✅ Back Images Save करें
    if (request.files && request.files.backImage && request.files.backImage.length > 0) {
        data.backImage = request.files.backImage.map(file => file.filename); // सभी filenames को array में स्टोर करें
    }
...
exports.view = async (request, response) => {
    try {
        ...
        response.status(200).json({ // डेटा मिल गया, तो 200 (OK) स्टेटस के साथ JSON रिस्पॉन्स देंगे।
            ...
            imagepath: "https://node-js-image-upload.onrender.com/uploads/Tabingcategory/",
